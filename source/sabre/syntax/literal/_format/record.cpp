...
SABRE_MM_FORMAT_PREFIX(Record, reader) {
  ...
  auto callback = [](Reader *reader) -> Piece * { return m_variable(reader, false); };
  auto *properties = m_delimited(reader, {std::move(callback), Lexer::Kind::PUNC_RBRACE});
