...
Sabre::Value::Any Sabre::Engine::Invoke::dynamic(Isolate *isolate, const Value::Any &target, const Args &args) {
...
Sabre::Value::Any Sabre::Engine::Invoke::native(Isolate *isolate, const Function::Native &native, const Args &args) {
...
Sabre::Value::Any Sabre::Engine::Invoke::jitted(Isolate *isolate, const Function::Jitted &jitted, const Args &args) {
  return m_jitted(isolate, jitted.info(), jitted.context(), args);
}

Sabre::Value::Any Sabre::Engine::Invoke::closure(Isolate *isolate, const Function::Closure &closure, const Args &args) {
  return m_closure(isolate, closure.info(), closure.context(), args);
}

Sabre::Value::Any Sabre::Engine::Invoke::inlined(Isolate *isolate, Function::Frame *frame, const Args &args) {
  return m_closure(isolate, frame->info(), frame->context(), args);
}

Sabre::Value::Any Sabre::Engine::Invoke::inlined(Isolate *isolate, const Machine::Info *info, const Args &args) {
  const auto *frame = isolate->frame()->as<Machine::Frame>();
  return m_jitted(isolate, info, frame->context(), args);
}

Sabre::Value::Any Sabre::Engine::Invoke::construct(Isolate *isolate, const Object::Class &prototype, const Args &args) {
...
bool Sabre::Engine::Invoke::m_validate(Isolate *isolate, const Function::Any &target, const Args &args) {
  return m_validate(isolate, target.arity(), args);
}

bool Sabre::Engine::Invoke::m_validate(Isolate *isolate, const Function::Info *info, const Args &args) {
  return m_validate(isolate, info->arity(), args);
}

bool Sabre::Engine::Invoke::m_validate(Isolate *isolate, size_t arity, const Args &args) {
...
Sabre::Function::Environ
Sabre::Engine::Invoke::m_initialize(Isolate *isolate, size_t leaked, const Function::Environ &parent) {
  return leaked ? Function::Environ(isolate, leaked, parent) : parent;
}

Sabre::Value::Any Sabre::Engine::Invoke::m_finalize(Isolate *isolate, const Frame *frame, const Value::Any &result) {
  if (!frame->modes().test(Interrupt::DISPOSE)) return result; // ignore
  return isolate->lifetimes()->close(isolate) ? result : Value::Failure();
}

Sabre::Value::Any Sabre::Engine::Invoke::m_closure(
...
Sabre::Value::Any Sabre::Engine::Invoke::m_jitted(
