#ifndef _SABRE_RUNTIME_ISOLATE_HPP
#define _SABRE_RUNTIME_ISOLATE_HPP
...
template <std::derived_from<Sabre::Value::Any> T>
Sabre::Handle::Local<T>::Local(Scope *scope) :
    m_isolate(scope->m_isolate), m_next(m_isolate->m_handles.push(m_reference())) {}

template <std::derived_from<Sabre::Value::Any> T>
Sabre::Handle::Local<T>::Local(Scope *scope, const Value::Any &value) :
...
template <std::derived_from<Sabre::Value::Any> T> Sabre::Handle::Local<T>::~Local() {
